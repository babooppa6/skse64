...
{
	UInt64	thunkAddress = (UInt64)GetIATAddr((UInt8 *)GetModuleHandle(NULL), "dinput8.dll", "DirectInput8Create");

	DICreate_RealFunc = (CreateDInputProc)*(DWORD *)thunkAddress;
	SafeWrite64(thunkAddress, (UInt64)Hook_DirectInput8Create_Execute);
}
