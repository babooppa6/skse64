...
bool IdentifyEXE(const char * procName, bool isEditor, std::string * dllSuffix, ProcHookInfo * hookInfo)
{
	...
	const UInt64 kCurVersion = 0x0001000000000000;	// 1.1.47.0

	if(version < kCurVersion)
	{
		PrintLoaderError("Please update to the latest version of %s.", (isEditor ? "the Creation Kit" : "SkyrimSE"));
	}
	else if(version > kCurVersion)
	{
		PrintLoaderError("You are using a newer version of %s than this version of SKSE64 supports. If the patch to this version just came out, please be patient while we update our code. In the meantime, please check http://skse.silverlock.org to make sure you're using the latest version of SKSE. (version = %016I64X %08X)", (isEditor ? "the Creation Kit" : "Skyrim"), version, PACKED_SKSE_VERSION);
	}
	else if(isEditor)
	{
		switch(hookInfo->procType)
	...
	else
	{
		switch(hookInfo->procType)
		{
			...
			case kProcType_Normal:
				...
				{
					hookInfo->hookCallAddr = 0x140786E52;
					hookInfo->loadLibAddr = 0x14425B4C8;
					*dllSuffix = "1_1_47";
	...
	// Remove base
	hookInfo->hookCallAddr -= 0x140000000;
	hookInfo->loadLibAddr -= 0x140000000;
