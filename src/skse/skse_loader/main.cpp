...
{
	gLog.OpenRelative(CSIDL_MYDOCUMENTS, "\\My Games\\Skyrim\\SKSE64\\skse64_loader.log");
	...
	else
	{
		...
		else
		{
			...
			char* procNameStr = (char*)&procName;

			if(procNameCheck != 1803113330)
			{
...
static bool DoInjectDLLThread(PROCESS_INFORMATION * info, const char * dllPath, bool sync)
{
	...
	if(process)
	{
		...
		if(hookBase)
		{
			...
			size_t	bytesWritten;
			WriteProcessMemory(process, (LPVOID)(hookBase + 12), dllPath, strlen(dllPath) + 1, &bytesWritten);

			UInt8	hookCode[12];

			hookCode[0] = 0x48;
			hookCode[1] = 0xB8;
			*((uintptr_t *)&hookCode[2]) = loadLibraryAAddr;
			hookCode[10] = 0xFF;
			hookCode[11] = 0xE0;

			WriteProcessMemory(process, (LPVOID)(hookBase), hookCode, sizeof(hookCode), &bytesWritten);

			HANDLE	thread = CreateRemoteThread(process, NULL, 0, (LPTHREAD_START_ROUTINE)hookBase, (void *)(hookBase + 12), 0, NULL);
			if(thread)
			{
				if(sync)
				{
